...
- Added Mac surrounding-quote answer normalization hardening:
  - imported answers copied with straight quote marks such as `"green park"`
    now match typed answers such as `green park`,
  - imported answers copied with curly quote marks such as `“green park”` also
    match typed answers,
...
- Added Mac imported multi-answer cell hardening:
  ...
  - Chinese `或` answer cells such as `green park 或 green parks` also expand
    into separate accepted variants,
  - multiline imported answer cells such as `green park\ngreen parks` also
    expand into separate accepted variants,
...
- Mac surrounding-quote answer normalization hardening:
  - `npx pnpm@9.15.4 --filter @ielts/shared test -- src/scoring.test.ts -t "surrounding quotes"`
    - Initially failed because accepted answer `"green park"` was not
      normalized to match typed answer `green park`.
    - Passed after straight and curly quote marks were normalized first and
      surrounding quote marks were stripped without removing internal
      apostrophes.
  - `npx pnpm@9.15.4 --filter @ielts/server test -- src/test/practiceRoutes.test.ts -t "surrounding quotes"`
    - Initially failed because the practice API marked `green park` incorrect
      against imported accepted answer `“green park”`.
    - Passed after the shared surrounding-quote normalization was used by
      practice answer scoring.
  - `npx pnpm@9.15.4 --filter @ielts/shared test -- src/scoring.test.ts -t "surrounding punctuation"`
    - Passed after the quote-normalization ordering change, confirming
      surrounding punctuation still normalizes correctly.
  - `npx pnpm@9.15.4 --filter @ielts/shared test -- src/scoring.test.ts`
...
- Mac imported-answer numbering-prefix hardening:
  ...
  - `npx pnpm@9.15.4 --filter @ielts/shared test -- src/scoring.test.ts -t "label prefixes"`
    - Initially failed because accepted answer `Answer: green park` was
      ...
      `green park`.
    - A follow-up red run failed for the Chinese label `答案：green park`
      because the full-width colon had no following space.
    - Passed after accepted-answer variants strip `Answer`, `Ans`, and `答案`
      labels with ASCII or full-width punctuation while preserving real answer
      text `answer`.
...
- Mac imported multi-answer cell hardening:
  - `npx pnpm@9.15.4 --filter @ielts/shared test -- src/scoring.test.ts -t "multi-answer"`
    ...
    - A later red run failed because Chinese `或` cells such as
      `green park 或 green parks` were not split.
    - A later red run failed because multiline answer cells such as
      `green park\ngreen parks` were normalized into one long literal answer.
    - Passed after semicolon-, pipe-, `or`-, Chinese `或`-, full-width
      semicolon-, enumeration-, and newline-delimited accepted-answer cells were
